...
function Navbar() {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn === 'true') {
      ...
      const fetchProducts = async () => {
        try {
          const response = await fetch(`http://localhost:5000/api/user/${userData.phone}`);
          if (!response.ok) {
  ...
  return (
    <div className={`flex justify-between items-center h-24 max-w-6xl mx-auto px-4`}>
      {load && <div className={`border fixed shadow rounded-md p-4 max-w-full min-h-full inset-0 z-50 w-full ${theme === 'dark' ? 'bg-gray-800 border-blue-300 ' : 'bg-white border-gray-200'} sm:ml-0`}>
      ...
      <ul className='flex-grow flex justify-start md:ml-16'>
        <li className='p-4 w-20 h-10 cursor-pointer font-semibold hover:text-black' onClick={() => handle_route('/')}>Home</li>
        <li className='p-4 w-20 h-10 cursor-pointer font-semibold hover:text-black'>Profile</li>
        <li className='md:block lg:block p-4 w-20 h-10 cursor-pointer font-semibold hover:text-black hidden'>Contact</li>
        <li className='px-10 w-20 h-10 hidden md:block lg:block'>
          <div className='flex flex-row'>
            <input type="search" placeholder="Search" className={`p-2 border rounded-md shadow-md focus:outline-none focus:ring-1`} />
      ...
      <ul className='hidden md:flex items-center gap-[2vw]'>
        ...
        {user ? (
         ...
         <div className="bg-amber-300 h-10 w-10 rounded-full p-2 flex items-center justify-center">
          ...
          <li className='flex items-center p-4 mx-2 cursor-pointer' onClick={() => navigate('/login')}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white p-8 rounded-lg shadow-lg w-11/12 max-w-md mx-auto">
            ...
            <div className="flex justify-between mt-4">
                <button
                    className="flex-1 bg-red-500 text-white py-2 rounded-md hover:bg-red-600 mr-2"
                    onClick={handleLogout}
                >
                    Logout
                </button>
                <button
                    className="flex-1 bg-slate-300 text-gray-500 hover:text-gray-700 py-2 rounded-md ml-2"
