...
function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch(
        "https://naturals-emart-project.onrender.com/api/login",
        {
          method: "POST",
          headers: {
  ...
  const fetchProducts = async (phone, token) => {
    try {
      const response = await fetch(
        `https://naturals-emart-project.onrender.com/api/Seller/${phone}`,
        {
          headers: {
            "Content-Type": "application/json",
    ...
    <div>
      ...
      <div className="min-h-screen flex justify-center items-center md:items-baseline bg-white p-4">
        <div className="bg-zinc-100 p-6 md:p-8 rounded-lg shadow-md max-w-xs md:max-w-sm w-full -mt-64 md:mt-12">
          ...
          <button
            onClick={handleLogin}
            className={`w-full bg-[#089B7D] text-white py-2 rounded-md shadow-md focus:outline-none ${
              isLoading ? "opacity-50" : "hover:bg-[#06795F]"
            }`}
            disabled={isLoading}
          >
