...
router.get("/admin/test-regular-requests", async (req, res) => {
  try {
    const regularProductRequests = await RegularProductRequest.find()
      .populate("name", "phone")
      .lean();
    ...
    res
      .status(500)
      .json({ message: "Error fetching regular product requests", error });
  }
});
...
router.get("/admin/requests", async (req, res) => {
  ...
  try {
    const [productRequests, regularProductRequests, buyerrequest] =
      await Promise.all([
    ...
    console.log(
      "Number of Regular Product Requests:",
      regularProductRequests.length
    );

    // Combine the results
    const allRequests = [
      ...productRequests,
      ...regularProductRequests,
      ...buyerrequest,
    ];
...
router.post("/admin/approve", async (req, res) => {
  ...
  if (!id || !status) {
    return res
      .status(400)
      .json({ message: "Missing required fields: id or status" });
  }
  ...
  try {
    ...
    if (status === "removed") {
      ...
      // Find the user and remove the product from their products array based on title and unique_id
      const user = await User.findOne({ phone });
      if (user) {
        user.products = user.products.filter(product => 
          !(product.title === productRequest.title && product.unique_id === productRequest.product_id)
        );
        await user.save();
      ...
      return res
        .status(200)
        .json({ message: "Product request removed successfully" });
    }
    ...
    if (user) {
      user.products.push({
        ...
        category_comes: productRequest.category_comes,
        unique_id: productRequest.product_id,
    ...
    res
      .status(200)
      .json({ message: "Product request status updated successfully" });
  } catch (error) {
    console.error("Error updating product request:", error);
    res
      .status(500)
      .json({ message: "Error updating product request status", error });
  }
});
...
router.post("/admin/approve_regular", async (req, res) => {
  ...
  try {
    ...
    if (status === "removed") {
      ...
      // Remove the product from the user's profile based on title and unique_id
      const user = await User.findOne({ phone });
      if (user) {
        user.products = user.products.filter(product => 
          !(product.title === productRequest.title && product.unique_id === productRequest.regular_product_id)
        );
        await user.save();
        console.log("Product removed from user:", user);
      }
    ...
    if (productRequest.status !== "pending") {
      return res
        .status(400)
        .json({ message: "Product request cannot be modified" });
    }
    ...
    if (user) {
      user.products.push({
        ...
        status: status,
        saleType: productRequest.saleType,
        unique_id: productRequest.regular_product_id,
        Approved_at: new Date(),
      });
      await user.save();
      console.log("Product added to user:", user);
...
router.post("/admin/approve_buyer", async (req, res) => {
  ...
  try {
    ...
    if (status === "removed") {
      ...
      // Find the user and remove the product based on title and unique_id
      const user = await User.findOne({ phone });
      if (user) {
        user.products = user.products.filter(product => 
          !(product.title === productRequest.title && product.unique_id === productRequest.unique_id)
        );
        await user.save();
        console.log("Product removed from user:", user);
      }
    ...
    if (user) {
      user.products.push({
        ...
        sector: productRequest.sector,
        sector_name: productRequest.sector_name,
        Approved_at: new Date(),
      });
      await user.save();
      console.log("Product added to user:", user);
